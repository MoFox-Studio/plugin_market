    ...
    async with session_scope() as session:
        ...
        for item in versions.items:
            if item.version == version:
                return item
        raise ApiError(404, "SKILL_VERSION_NOT_FOUND", "Skill version not found.", {"skill_id": skill_id, "version": version})
...
async def download_skill_version(skill_id: str, version: str) -> FileResponse:
    """Download a skill version zip package."""
...
async def delete_skill_comment(skill_id: str, comment_id: int, request: Request) -> dict[str, bool]:
    """Delete a comment owned by the viewer or by an admin."""
...
async def toggle_skill_like(
    ...
    """Toggle the viewer's like on a skill."""
...
async def set_skill_trust_level(
    ...
    """Set the trust badge shown for a skill."""
